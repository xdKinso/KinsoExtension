...
export class Interceptor extends PaperbackInterceptor {
  ...
  override async interceptResponse(
    ...
    if (response.status === 523) {
      ...
      console.error(
        `[MangaPark] This usually means rate limiting is too aggressive or origin is down`,
      );
    ...
    if (isCDNImageUrl(request.url)) {
      ...
      if ((response.status >= 500 || data.byteLength === 0) && currentServer) {
        ...
        console.log(
          `[MangaPark] Server ${currentServer} marked as failed (status: ${response.status})`,
        );
      }
...
export {
  getServerFromUrl,
  replaceServer,
  getNextWorkingServer,
  getNextUntriedServer,
  isCDNImageUrl,
  SERVER_PATTERN,
  CDN_SERVERS,
  failedServers,
